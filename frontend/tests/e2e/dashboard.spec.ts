...
test.describe("Mission Control Dashboard", () => {
  test.beforeEach(async ({ page }) => {
    await page.route("**/health", (route) =>
      route.fulfill({ json: { status: "ok", obsidian_backend: true } })
    );
    await page.route("**/dashboard/state", (route) =>
      route.fulfill({
        json: { locks: [], recent_activity: [], agents: [], tasks: [], stalls: [] },
      })
    );
  });
  ...
  test("displays activity stream", async ({ page, context }) => {
    ...
    await expect(
      page.locator("table th:has-text('Agent')")
    ).toBeVisible();
  });
});
