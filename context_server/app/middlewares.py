...
class PolicyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if tool in ["append_implement", "log_decision"]:
            _expected_version = request.headers.get("X-Expected-Version")
            # Note: actual comparison requires fetching the target note, 
            # we rely on rejectIfContentPreexists in obsidian_backend for idempotency currently.
